...
type ObjectSchemaToTS<T> = T extends {
  properties: infer Props extends Record<string, unknown>;
}
  ? T extends { required: infer Required extends unknown[] }
    ? Omit<
        {
          [Key in keyof Props as Key extends Required[number]
            ? Key
            : never]: JSONSchema2TS<Props[Key]>;
        } & {
          [Key in keyof Props as Key extends Required[number]
            ? never
            : Key]?: JSONSchema2TS<Props[Key]>;
        },
        never
      >
    : {
        [Key in keyof Props]?: JSONSchema2TS<Props[Key]>;
      }
  : Record<string, unknown>;

type ArraySchemaToTS<T> = T extends { items: infer Items }
  ? JSONSchema2TS<Items>[]
  : unknown[];
...
type JSONSchema2TS<T> = T extends { type: infer Type }
  ? Type extends 'object'
    ...
    ? T['enum'][number]
    : Type extends keyof PrimitiveMap
    ? PrimitiveMap[Type]
    : ArraySchemaToTS<T>
  : never;
